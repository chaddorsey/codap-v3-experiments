...
jest.mock('../../../v3/src/components/graph/plots/case-plot/case-plot', () => ({
  CasePlot: ({ pixiPoints, abovePointsGroupRef }) => {
    // For testing purposes, we'll extract data from pixiPoints mock
    const data = pixiPoints?.data || [];
    
    // Calculate case positions (randomly distributed in a grid)
    const casePositions = data.map((point) => {
      return {
        ...point,
...
jest.mock('../../../v3/src/hooks/use-data-set-context', () => ({
  useDataSetContext: () => ({
    getItem: (id) => ({ __id__: id, x: 0.5, y: 0.5 }),
...
import { CasePlot } from '../../../v3/src/components/graph/plots/case-plot/case-plot';
...
describe('Case Plot Component', () => {
  ...
  const createMockPixiPoints = (data) => {
    ...
    const mockPixi = {
      ...
      destroy: jest.fn(),
      forEachSelectedPoint: jest.fn((callback) => {
